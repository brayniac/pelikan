...
impl Listener {
	...
	pub fn new(config: Arc<PingserverConfig>, sender: Sender<Session>) -> Result<Self, std::io::Error> {
		...
		poll.register(&listener, Token(0), Ready::readable(),
			PollOpt::edge()).map_err(|e| {
				error!("{}", e);
				std::io::Error::new(std::io::ErrorKind::Other, "Failed to register listener with epoll")
			})?;
	...
	pub fn run(&mut self) {
		...
		let timeout = Some(std::time::Duration::from_millis(self.config.server().timeout() as u64));
