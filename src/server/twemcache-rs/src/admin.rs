...
impl Admin {
    ...
    pub fn new(config: Arc<Config>, metrics: Arc<Metrics>) -> Result<Self, Error> {
    ...
    pub fn get_rusage(&self) {
        ...
        if unsafe { libc::getrusage(libc::RUSAGE_SELF, &mut rusage) } == 0 {
            ...
            self.metrics().record_gauge(Stat::RuMaxrss, rusage.ru_maxrss);
            ...
            self.metrics().record_counter(Stat::RuMinflt, rusage.ru_minflt as u64);
            self.metrics().record_counter(Stat::RuMajflt, rusage.ru_majflt as u64);
            self.metrics().record_counter(Stat::RuNswap, rusage.ru_nswap as u64);
            self.metrics().record_counter(Stat::RuInblock, rusage.ru_inblock as u64);
            self.metrics().record_counter(Stat::RuOublock, rusage.ru_oublock as u64);
            self.metrics().record_counter(Stat::RuMsgsnd, rusage.ru_msgsnd as u64);
            self.metrics().record_counter(Stat::RuMsgrcv, rusage.ru_msgrcv as u64);
            self.metrics().record_counter(Stat::RuNsignals, rusage.ru_nsignals as u64);
            self.metrics().record_counter(Stat::RuNvcsw, rusage.ru_nvcsw as u64);
            self.metrics().record_counter(Stat::RuNivcsw, rusage.ru_nivcsw as u64);
        }
    }
}
                ...
                match parse(&mut session.read_buffer) {
                    ...
                    Ok(request) => match request {
                        Request::Stats => {
                            ...
                            data.sort();
                            for line in data {
                                session.write_buffer.extend_from_slice(line.as_bytes());
                            }
                            session.write_buffer.extend_from_slice(b"END\r\n");
                            let _ = self
                                .metrics
