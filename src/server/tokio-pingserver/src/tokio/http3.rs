...
pub async fn run(config: Arc<Config>) {
    let cert_file = config.tls().certificate().expect("no certificate configured");
    let cert_content = std::fs::read(cert_file).expect("failed to read cert");
    let cert = CertificateDer::from(cert_content);

    let key_file = config.tls().private_key().expect("no private key configured");
    ...
    let endpoint = quinn::Endpoint::server(server_config, config.listen()).expect("failed to start quic endpoint");

    loop {
        if let Some(incoming_conn) = endpoint.accept().await {
            tokio::spawn(async move {
                if let Ok(conn) = incoming_conn.await {
                    if let Ok(mut conn) = h3::server::Connection::new(h3_quinn::Connection::new(conn)).await {
                        loop {
                            if let Ok(Some((request, mut stream))) = conn.accept().await {
